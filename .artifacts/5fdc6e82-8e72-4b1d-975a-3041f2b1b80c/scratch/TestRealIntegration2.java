...
public class TestRealIntegration2 {
    public static void main(String[] args) {
        try {
            ...
            if (idx != -1) {
                ...
                int searchIdx = 0;
                while (true) {
                    int itagIdx = json.indexOf("\"itag\":140", searchIdx);
                    if (itagIdx == -1) break;
                    
                    int urlIdx = json.indexOf("\"url\":\"", itagIdx);
                    if (urlIdx != -1 && (urlIdx - itagIdx < 800)) {
                        int urlEnd = json.indexOf("\"", urlIdx + 7);
                        String streamUrl = json.substring(urlIdx + 7, urlEnd).replace("\\u0026", "&");
                        if (streamUrl.startsWith("http") && !streamUrl.contains("ytimg.com")) {
                            System.out.println("VALID AUDIO STREAM URL: " + streamUrl);
