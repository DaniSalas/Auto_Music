...
public class TestRealWebpageScrape12 {
    public static void main(String[] args) {
        try {
            ...
            if (idx != -1) {
                ...
                if (itagIdx != -1) {
                    // search backwards from itagIdx for "url":"
                    int urlIdx = json.lastIndexOf("\"url\":\"", itagIdx);
                    System.out.println("urlIdx backwards from itag 140: " + urlIdx);
                    if (urlIdx != -1) {
                        int urlEnd = json.indexOf("\"", urlIdx + 7);
                        String streamUrl = json.substring(urlIdx + 7, urlEnd).replace("\\u0026", "&");
                        System.out.println("STREAM URL BACKWARDS: " + streamUrl);
