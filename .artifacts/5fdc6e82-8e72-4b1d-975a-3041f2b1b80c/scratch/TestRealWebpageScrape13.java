...
public class TestRealWebpageScrape13 {
    public static void main(String[] args) {
        try {
            ...
            if (idx != -1) {
                ...
                if (itagIdx != -1) {
                    // find the preceding { before itagIdx
                    int bracketIdx = json.lastIndexOf("{", itagIdx);
                    if (bracketIdx != -1) {
                        String objStr = json.substring(bracketIdx, itagIdx + 100);
                        System.out.println("Object snippet: " + objStr);
                        int urlIdx = objStr.indexOf("\"url\":\"");
                        if (urlIdx != -1) {
                            int urlEnd = objStr.indexOf("\"", urlIdx + 7);
                            String streamUrl = objStr.substring(urlIdx + 7, urlEnd).replace("\\u0026", "&");
                            System.out.println("CORRECT ITAG 140 AUDIO URL: " + streamUrl);
