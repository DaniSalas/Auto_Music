...
public class TestRealWebpageScrape17 {
    public static void main(String[] args) {
        try {
            ...
            if (idx != -1) {
                ...
                if (itagIdx != -1) {
                    // Let's print the entire json object block containing itag 140 (from preceding { to following })
                    int startBrace = json.lastIndexOf("{", itagIdx);
                    int endBrace = json.indexOf("}", itagIdx);
                    System.out.println("Block: " + json.substring(startBrace, endBrace + 1));
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
