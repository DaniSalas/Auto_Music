...
public class TestWebpageAudioUrl3 {
    public static void main(String[] args) {
        try {
            ...
            if (idx != -1) {
                ...
                System.out.println("JSON snippet around audio/mp4:");
                int aIdx = json.indexOf("audio/mp4");
                if (aIdx != -1) {
                    System.out.println(json.substring(Math.max(0, aIdx - 200), Math.min(json.length(), aIdx + 400)));
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
