    ...
    private inner class LibrarySessionCallback : MediaLibrarySession.Callback {
        ...
        override fun onSetMediaItems(mediaSession: MediaSession, controller: MediaSession.ControllerInfo, mediaItems: MutableList<MediaItem>, startIndex: Int, startPositionMs: Long): ListenableFuture<MediaSession.MediaItemsWithStartPosition> {
            ...
            serviceScope.launch {
                ...
                if (alreadyInTimeline && currentItems.size > 1) {
                    Log.d("MusicService", "Preserving current timeline for $targetMId at index $existingIndex")
        ...
        override fun onGetChildren(session: MediaLibrarySession, browser: MediaSession.ControllerInfo, parentId: String, page: Int, pageSize: Int, params: LibraryParams?): ListenableFuture<LibraryResult<ImmutableList<MediaItem>>> {
            ...
            serviceScope.launch {
                try {
                    if (parentId == "ROOT") {
                        val items = repository.allPlaylists.first().map { MediaItem.Builder().setMediaId("PLAYLIST_${it.id}").setMediaMetadata(MediaMetadata.Builder().setTitle(it.name).setIsBrowsable(true).setIsPlayable(false).setMediaType(MediaMetadata.MEDIA_TYPE_PLAYLIST).setExtras(Bundle().apply { putInt("android.media.browse.CONTENT_STYLE_BROWSABLE_HINT", 1) }).build()).build() }
                        ...
                        val songs = repository.getSongsInPlaylist(pid).first()
                        future.set(LibraryResult.ofItemList(songs.map { createMediaItem(it, pid) }, params))
